...
used, still only C<OP_SIGNATURE>.
...
security in the collision resolution instead.
...
The old cperl C<-Dfortify_inc> security feature was now renamed to
C<-Ddefault_inc_excludes_dot>

Because the testing and make process for perl modules does not work well with
C<.> missing from @INC, cperl still supports the environment variable
...
The C<dtrace> build process was improved I<[perl #130108]>:
...
cperl 5.25.2c, merged 5 perl5 releases from 5.25.3 to 5.25.9 with
...
improvements that became cperl 5.25.3c:
