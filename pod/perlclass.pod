...
=item Max 65536 fields

L<Too many fields|perldiag/"Too many fields">:
cperl 5.27.2 added a limitation of max. 65536 fields per class.

=item field index fixups with roles
...
class, the method is not yet fixed up to the new indices. 
A temp. solution would be to change the ordering of the roles, or to use
the C<< $self->field >> method syntax in the role method.  
This requires the not yet finished inliner.
Currently we can only alias composed role methods and we don't change
the ordering of the fields. 
See L<[cperl #311]|https://github.com/perl11/cperl/issues/311>

=item eval 'class {}' fails

Cannot be created in an eval block or subroutine.
The pad lookup is still global and not per optional CvPADLIST.

=item Array and hash fields cannot be parsed yet.
...
It is not yet the fastest field and method accessor, though
theoretically it should be.  It still has about the same speed as
Mouse, though the new oref ops would allow to skip refcounting of
fields and store them as native offsets into the object. Some oref
compile-time optimizations are not yet implemented.
