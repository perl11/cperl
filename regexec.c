...
#define PUSH_SCAN_STATE_GOTO(state, node, input, eol, sr0)  \
    pushinput = input; \
    pusheol = eol;     \
    pushsr0 = sr0; \
    st->resume_state = state; \
...
#define PUSH_YES_SCAN_STATE_GOTO(state, node, input, eol, sr0)   \
    pushinput = input; \
    pusheol = eol;     \
    pushsr0 = sr0; \
    st->resume_state = state; \
...
#define ST st->u.trie
	...
	case TRIE_next_fail: /* we failed - try next alternative */
	    ...
	    if ( ST.accepted > 1 || has_cutgroup || ST.jump ) {
                PUSH_SCAN_STATE_GOTO(TRIE_next, scan, (char*)uc, loceol,
                                     script_run_begin);
		NOT_REACHED; /* NOTREACHED */
	    }
...
#define ST st->u.branch
	...
	case BRANCH:	    /*  /(...|A|...)/ */
	    ...
	    if (has_cutgroup) {
                PUSH_YES_SCAN_STATE_GOTO(BRANCH_next, scan, locinput, loceol,
                                         script_run_begin);
	    } else {
                PUSH_SCAN_STATE_GOTO(BRANCH_next, scan, locinput, loceol,
                                     script_run_begin);
	    }
	    NOT_REACHED; /* NOTREACHED */
