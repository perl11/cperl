...
sub __adjust_html_h_levels ($self) :method {
...
sub _add_top_anchor ($self, $text_r) :method {
...
sub version_tag_comment ($self) :method {
  return sprintf
...
sub _do_middle_main_loop ($self) :method {
  my $fh = $self->{'output_fh'};
  my $tagmap = $self->{'Tagmap'};
...
sub do_section ($self, $name, $token) :method {
  return $name;
}

sub do_link ($self, $token) :method {
...
sub do_url_link ($self, $token) :method { $token->attr('to') }

sub do_man_link ($self, $link) :method {
  my $to = $link->attr('to');
...
sub do_pod_link ($self, $link) :method {
...
sub section_escape ($self, str $section) :method {
  return $self->section_url_escape(
    $self->section_name_tidy($section)
  );
}

sub section_name_tidy ($self, str $section) :method {
  $section =~ s/^\s+//;
  $section =~ s/\s+$//;
  $section =~ tr/ /_/;
  if ($] ge 5.006) {
...
sub section_url_escape  ($self, @args) :method { $self->general_url_escape(@args) }
sub pagepath_url_escape ($self, @args) :method { $self->general_url_escape(@args) }
sub manpage_url_escape  ($self, @args) :method { $self->general_url_escape(@args) }

sub general_url_escape ($self, str $string) :method {
...
sub resolve_pod_page_link ($self, $it?) :method {
...
sub resolve_pod_page_link_singleton_mode ($self, $it?) :method {
...
sub resolve_pod_page_link_batch_mode ($self, str $to) :method {
...
sub batch_mode_rectify_path ($self, $pathbits) :method {
...
sub resolve_man_page_link ($self, str $to, $frag) :method {
...
sub resolve_pod_link_by_table ($self, $to?, $section?) :method {
...
sub linearize_tokens ($self, @tokens) :method {
  my $out = '';
...
sub unicode_escape_url ($self, str $string) :method {
