...
"sub bla {}">, changing the definition of a function at run-time, just
...
filesystem directly without sanitation or proper checks.
...
Provide a special C<:const> hash table type via optimizations to
...
as planned by Damian Conway, i.e. they cannot be changed later. This
enables the compiler to inline them automatically when its
worthwhile. e.g. when they are small enough.  They are either defined
in the new style: C<class .. { my method .. }> or old style: C<package
.. { my sub .. }>
...
Starting with cperl (based on Perl 5.22) L<Carp> is now a builtin XS module.
There's no need to require it.  F<Carp.pm> is only provided for
documentation, C<$INC{'Carp.pm'} = 'xsutils.c'>
...
=head3 DynaLoader 2.00c as builtin

Starting with cperl (based on Perl 5.22) L<DynaLoader> has no perl code
anymore, F<DynaLoader.pm> was rewritten as F<dlboot.c>.
...
TODO:
Provide a C<XSLoader::load_file($module, $modlibname, ...)> function and
make L<XSLoader> a builtin also. C<...> is passed to the loaded XS function
...
They will be compiled to a shared library.
