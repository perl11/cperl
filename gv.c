...
Perl_gv_autoload_pvn(pTHX_ HV *stash, const char *name, STRLEN len, U32 flags)
{
	...
	if (SvPOK(cv)) { /* Ouch! */
	    ...
	    ulen = SvCUR(tmpsv);
	    SvCUR_set(tmpsv, ulen+1); /* include null in string */
...
{
    char *tmpfullbuf = NULL; /* only malloc when smallbuff is not large enough */
...
Perl_amagic_call(pTHX_ SV *left, SV *right, int method, int flags)
{
...

    {
        ...
        I32 oldmark, nret;
                 ...
                 */
        U8 gimme = (force_scalar || OP_TYPE_IS_NN(PL_op, OP_MULTICONCAT))
                    ? G_SCALAR : GIMME_V;
