...
or:
    ...
    make -s -j4 test && make -s install DESTDIR=`pwd`/../cperl-5.26.1-inst

    mkdir ../cperl-5.26.1
    cp -rp .git ../cperl-5.26.1/
    cd ../cperl-5.26.1
    git reset --hard
    git clean -dxf
    ...
    for t in deb rpm osxpkg solaris pkgin; do
      fpm -s dir -t $t -C cperl -p ../cperl-5.26.1-inst/ \
        --version 5.26.1 \
        -m rurban@cpan.org \
...
internal test files.
    ...
    mkdir ../cperl-5.26.1-build
    cp -rp .git ../cperl-5.26.1-build/
    cd ../cperl-5.26.1-build
    git reset --hard
    git clean -dxf
...
.rpm, osx .pkg, bsd and solaris packages, but not yet windows .msi
    ...
    fpm -s dir -t deb -n cperl -v 5.26.1-1 -m 'rurban@cpan.org' \
...
Add the debs to the pool:
    ...
    aptly repo add cperl ../../cperl_5.26.1-1_{i386,amd64}.deb
