...
=head2 Role composition was enhanced
...
simple GV aliasing.  The other methods are now cloned and the C<$self>
type is adjusted to the new class for the created new method, because
v5.30 started argument type checking at run-time.

But cperl still cannot adjust field indices when composing roles, when
...
Added the new setstate and keepstate ops for the inliner,
variants of nextstate.
...
=item L<Pod::Simple>  4.38c

Merge with updates from 3.38
...
L<YAML::XS> is no longer distributed with the cperl core distribution.
It was replaced with L<YAML::Safe>.
...
Unescaped left brace in regex is deprecated here (and will be fatal in Perl 5.32), passed through in regex; marked by S<<-- HERE> in m/%s/
...
cperl still cannot adjust field indices when composing roles, when the
indices of the lexical variables of the two methods do not match.
When copying methods from roles, the pad's might be different.
This needs the inliner, which is not yet stable.
