...
S_remove_ppf_prefix(const char *lnm, char *eqv, unsigned short int eqvlen)
{
    if (*((int *)lnm) == *((int *)"SYS$")                    &&
        ...
        ( (lnm[4] == 'O' && strEQc(lnm,"SYS$OUTPUT"))  ||
          (lnm[4] == 'I' && strEQc(lnm,"SYS$INPUT"))   ||
          (lnm[4] == 'E' && strEQc(lnm,"SYS$ERROR"))   ||
          (lnm[4] == 'C' && strEQc(lnm,"SYS$COMMAND")) )  ) {
...
int_fileify_dirspec(const char *dir, char *buf, int *utf8_fl)
{
	...
	*utf8_fl = 0;
     ...
     */
    if (dirlen >= 2 && strEQc(trndir+dirlen-2,".]")) {
      trndir[--dirlen] = '\0';
      trndir[dirlen-1] = ']';
    }
    if (dirlen >= 2 && strEQc(trndir+dirlen-2,".>")) {
...
/* fix-me, can not scan EFS file specs backward like this */
    ...
    if (hasfilename || !cp1) { /* filename present or not VMS */
      ...
      else if (dirlen >= 7 && strEQc(&trndir[dirlen-7],"/000000")) {
	char * ret_chr;
...
<<<<<<< HEAD
	    ...
	    if (strEQc(vmspath, "bin")) {
||||||| parent of 1d60dc3fde... Make VMS CRTL features work for embedders.
        if (!islnm && !decc_posix_compliant_pathnames) {
	    if (strEQ(vmspath, "bin")) {
=======
        if (!islnm && !DECC_POSIX_COMPLIANT_PATHNAMES) {
	    if (strEQ(vmspath, "bin")) {
>>>>>>> 1d60dc3fde... Make VMS CRTL features work for embedders.
	        /* bin => SYS$SYSTEM: */
		islnm = vmstrnenv("SYS$SYSTEM:", esa, 0, fildev, 0);
	    }
	    else {
	        /* tmp => SYS$SCRATCH: */
		if (strEQc(vmspath, "tmp")) {
