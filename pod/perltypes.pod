...
type-optimizations.  Esp. useful are declarations of const packages
and the @ISA inheritance tree to optimize method calls at
compile-time.
const sets the internal READONLY flags for the variable, if it's
...
Declaration of a const variable without initialization B<XXX> 
    1. is disallowed I<(preferred)>
    2. leads to a warning
    3. stores undef

External modules, such as L<coretypes>, L<types>, L<typesafety> or
L<Moose> implement their type checks for their types during execution
of L<perlmod/CHECK> blocks.
...
implement respective types. The only type optimization currently in
effect is constant folding, and the change of compile-time static
method calls, determined either by name or by type and const-ness of
all searched packages to static function calls.  Thus the dynamic
method search, in which object this method is implemented is
avoided. This is about ~10% faster.
...
Right-hand-side expressions with resolve at compile-time to constant integers
may be optimized to a CONST value, and left-hand-side numeric ops may be optimized
to use their optimized <i_> counterparts.
Note that i_ ops do not overflow, the integer values just wrap around.
...
Since C<&MyClass::new> is lexically defined, the compiler may statically
optimize all sub and method calls to C<&MyClass::new> to an improved entersub
(the CV being a pad) without namespace lookup and dynamic method
resolution, without having to const C<%MyClass::> and const C<@MyClass::ISA>.
...
As convenience for module authors it be handy to allow declarations, like
