...
inside the class each has field is declared as lexical my variable,
...
Fields are always compile-time composed, even with C<is CLASS> inheritance.
...
getters.  You may define you own accessor methods.
...
Note that internally an array ref holds the fields, but to the user it
looks like a hash ref. The compiler converts the C<$self> hashref
either to a compile-time array access or to a new run-time C<has> op,
which performs the hash lookup of the field value. Note that it is 
not a 5.8 pseudo-hash, it just looks like one.

    my CLASS $obj = new CLASS;
    $obj->{field}; 	# -> $obj->[0]

    my $field = 'field';
    $obj->{$field};	# -> has $obj, $field

With method accessors it is similar, but you can override them. When
the compiler can ensure that a field accessor method is static, it is
inlined to the array ref.
It is not static when the class is open, or the object is not typed.

    my CLASS $obj = new CLASS;
    $obj->field; 	# -> $obj->[0]

    my $obj = new CLASS;
    $obj->field; 	# call field($obj)
