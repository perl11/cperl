...
C<6 * 5 == 30>.
Some special cperl/perl6 unicode operators have higher precedence
than normal operators. So far only the fraction C<U+2044> operator.
...
values only, not array values.
    ...
    left	-> →
    left        ⁄ fraction U+02044
    nonassoc	++ --
    right	** ² ³
    right	! ~ \ and unary + and -
    left	=~ !~
    left	* / % x ÷ ⋅ sdot U+022C5 mdot U+2219
    left	+ - .
    left	<< >>
...
"C<< -> >>" or cperl C<→> is an infix dereference operator, just as it
is in C and C++.  If the right side is either a C<[...]>, C<{...}>, or
a C<(...)> subscript, then the left side must be either a hard or
...
The cperl unicode constant superscript operators C<⁰>, C<¹>, C<²> -
C<⁹> are also valid. See L</Unicode Operators>.
...
The unicode C<⋅> U+022C5 (dot operator) and C<∙> U+2219 (multiplication dot)
also multiply two numbers, when C<use utf8> and C<use feature 'unicode_strings'>
(eg. via use v5.26) are in effect.

Binary C<"/"> divides two numbers.
X</> X<slash>

The unicode C<⁄> U+02244 (fraction) divides two numbers, when C<use
utf8> and C<use feature 'unicode_strings'> (eg. via use v5.26) are in
effect, but the precedence is the highest of all arithmetic operators.

Binary C<"%"> is the modulo operator, which computes the division
remainder of its first argument with respect to its second argument.
Given integer operands C<$m> and C<$n>: If C<$n> is positive, then
S<C<$m % $n>> is C<$m> minus the largest multiple of C<$n> less than
or equal to C<$m>.

In perl5 without C<use integer>, or with C<no integer>:
If C<$n> is B<negative>, then S<C<$m % $n>> is C<$m> minus the
smallest multiple of C<$n> that is not less than C<$m> (that is, the
result will be less than or equal to zero).

With B<use integer> or with B<cperl> and typed integer variables or
integer constants the definition of C<"%"> with one negative argument
is different and behaves like with use integer. It uses the
implementation from libc.  See L<perlcperl/"negative integer modulo">.
Use untyped variables and C<no integer> to use the original perl5 definition
of modulo.

If the operands C<$m> and C<$n> are floating point values and the
absolute value of C<$n> (that is C<abs($n)>) is less than S<C<(UV_MAX
+ 1)>>, only the integer portion of C<$m> and C<$n> will be used in
the operation (Note: here C<UV_MAX> means the maximum of the unsigned
integer type).  If the absolute value of the right operand
(C<abs($n)>) is greater than or equal to S<C<(UV_MAX + 1)>>, C<"%">
computes the floating-point remainder C<$r> in the equation S<C<($r =
$m - $i*$n)>> where C<$i> is a certain integer that makes C<$r> have
the same sign as the right operand C<$n> (B<not> as the left operand
C<$m> like C function C<fmod()>) and the absolute value less than that
of C<$n>.  Note that when S<C<use integer>> is in scope, C<"%"> gives
...
but it will execute faster.  X<%> X<remainder> X<modulo> X<mod>
...
In B<cperl> several mathematical, arrow or logical unicode operators
can also be used, similar to perl6.  They are only in effect with
C<use utf8> and C<use feature 'unicode_strings'> (eg. via use v5.26).

It's precedence is mostly the same is the non-unicode counterparts,
just the fraction operator binds higher then div C</>.

However, there are no perl6 ascii counterparts, the so-called "texas"
operators. L<https://docs.perl6.org/language/unicode_texas.html>

=over

=item C<⇒> U+021D2 fatcomma =>

=item C<→> U+02192> arrow ->

=item C<⁄> U+02044 fraction
...
=item C<¹>, C<²>, ... pow constants from 0..9 as superscripts
...
Binary C<< "<=" >> or C<≤> returns true if the left argument is
numerically less than or equal to the right argument.  X<< <= >>

Binary C<< ">=" >> or C<≥> returns true if the left argument is
numerically greater than or equal to the right argument.  X<< >= >>
...
Binary C<< "==" >> returns true if the left argument is numerically
...
Binary C<< "<=>" >> or C<⇔> returns -1, 0, or 1 depending on whether the left
