...
Perl pragmas.  Builtin types in perl5 had all uppercase names,
with cperl and perl6 types are named conventionally.
To prevent confusion, you may wish to avoid all uppercase names as well.
...
See L<perlmod/"Perl Modules">, L<perltypes> and L<perlclass>
...
That's why you must now invoke it as C<CORE::dump()>.
...
the hash (but not array) in list context.
...
which C<ref> will return the name of the subclass.
Also beware that class names can clash with the built-in type names (described below).
...
C<GLOB>, C<LVALUE>, or C<REGEXP>, depending on the kind of value the
scalar currently has.  But note that C<qr//> scalars are created
already blessed, so C<ref qr/.../> will likely return C<Regexp>.
Beware that these built-in type names can also be used as class names,
so C<ref> returning one of these names doesn't unambiguously indicate
that the referent is of the kind to which the name refers.
...
See also L<perlref>, L<perlobj> and L<perltypes>.
...
returned.
In older perls the subroutine received a meaningless argument
(in fact always the numeric value zero) as C<$_[0]>. It should be the
reference to the subroutine itself.
