...
const struct op_private_by_op op_private_names[] = {
    ...
    {OP_PADSV, C_ARRAY_LENGTH(op_pad_names), op_pad_names },
    {OP_PADAV, C_ARRAY_LENGTH(op_pad_names), op_pad_names },
    {OP_PADHV, C_ARRAY_LENGTH(op_pad_names), op_pad_names },
    {OP_PADCV, C_ARRAY_LENGTH(op_pad_names), op_pad_names },
    {OP_PADANY, C_ARRAY_LENGTH(op_pad_names), op_pad_names }
};
	...
	if (optype == OP_NULL) {
	    ...
	    Perl_dump_indent(aTHX_ level, file, "TARG = %ld\n", (long)o->op_targ);
            if (optype == OP_PADSV ||
                ...
                optype == OP_PADCV ||
                optype == OP_PADANY) {
                if (o->op_private & (OPpPAD_CONST|OPpPAD_CONSTINIT))
                    Perl_dump_indent(aTHX_ level, file, "CONST = %s\n", SvPEEK(PAD_SVl(o->op_targ)));
            }
        }
    }
#ifdef DUMPADDR
