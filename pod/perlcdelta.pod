...
The latest msys2 64bit gcc-9 compiler fails on two tests: F<op/sort.t> and
F<op/signatures.t>, similar to the darwin gcc-9 where the optimizer
generates wrong code in XSConfig.
