...
C<afl-fuzz -x Porting/afl.dict> dictionary. See L<perlhacktips/afl-fuzz>.

=head2 F<appveyor-deploy.sh>

Helper script to package a binary release package on windows,
for mingw and cygwin, 32- and 64-bit, tagged or nightly releases.
Creates reproducible build tarballs.
The MSVC counterpart happens in F<t\appveyor-smoke.bat>
...
=head2 F<do-conf-clean>

Build helper script to clean all. Called by F<do-conf-cperl-release>.

=head2 F<do-conf-cperl-release>

Build helper script to build a binary release.

=head2 F<do-make-cperl-release>

Build helper script to package a binary release package, built with
F<do-conf-cperl-release>.
Add an optional revision argument.

=head2 F<do-make-srctarball>

Build helper script to package a src tarball.
...
DTrace script for probes of loading external files via do or
require. See L<perldtrace>.
...
DTrace script for op probes. See L<perldtrace>.

=head2 F<opprof.pl>

Post-process tmp files with op_type stats generated via C<-DUSE_OPPROF>.
...
=head2 F<perl_version>

Build helper script to return the perl version from F<patchlevel.h>.
Called by F<do-conf-cperl-release>, F<do-make-cperl-release> and
F<do-make-srctarball>.
...
=head2 F<release_cperl.pod>

Releasing a new version of cperl 5.x. Creating the native packages and
publishing them.
...
DTrace script for sub call probes. See L<perldtrace>.
