...
This document describes differences between the cperl 5.26.0 and the cperl
5.28.0 release.
...
=head2 Named anonymous CV - usenamedanoncv

Support named anonymous CV names, i.e. orphaned or interim subroutine names
are not changed to C<__ANON__> anymore, but a C<@> character is appended.
The C<@> character is also used to identify numbered special blocks like C<BEGIN@1>
in L<Devel::NYTProf> and later to name and identify B<multi> methods (polymorphism).
...
=head2 C<'> is a valid part of a subroutine name


Accept C<'> as valid subroutine name character in declarations,
if in the middle, just don't expand it to C<::>.
This allows: C<sub don't { ... }> but not C<sub do' {}> nor C<sub 'do {}>.

Unfortunately using this name is not yet as easy as before, when the
C<'> was expanded to C<::>. C<print don't;> is illegal, as the quote
looks for the 2nd string delimiter pair, and doesn't yet check for the
existing subroutine name.

In XS you can check for the define C<PERL_NO_QUOTE_PKGSEPERATOR> how
the quote is handled.
...
the cperl F<feature/gh138-iter-and> branch from 
L<[cperl #138]|https://github.com/perl11/cperl/issues/138>.
...
cperl added the optimization to skip expanding C<'> to C<::>. 
The C<'> as valid perl4-style package seperator was removed.
In XS you can check for the define PERL_NO_QUOTE_PKGSEPERATOR if the quote as
package seperator is disallowed.
C<'> is now a valid part of a subroutine name.
...
Minor updates from p5p, but they are still distributing a broken module,
bumping their version higher than ours. And worse, their version creates
broken CPAN modules.
...
Fix t/subname.t and t/exotic_names.t for unstrict names.
...
added head/tail
honor PERL_NO_QUOTE_PKGSEPERATOR
merged %DB::sub tests
merge with upstream %DB::sub support (main:: or not, utf8)
...
Improved some tests which depend in counting (eval \d)'s to use regex.
Print the compiled regex when failing, not just the expected string.
In this case F<t/lib/warnings/pad> Storable used in Unicode::UCD, used
by utf8 added another eval.
