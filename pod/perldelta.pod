...
L<[perl #126632]|https://rt.perl.org/Ticket/Display.html?id=126632>

=item *

You can now build perl with MSVC++ on Win32 using GNU Make.
L<[perl #126632]|https://rt.perl.org/Ticket/Display.html?id=126632>
...
C<alignbytes> could cause alignment errors on debugging builds.

L<[perl #127894]|https://rt.perl.org/Ticket/Display.html?id=127894>
...
L<[perl #126045]|https://rt.perl.org/Ticket/Display.html?id=126045>
...
You can now build perl with GNU Make and GCC.
L<[perl #123440]|https://rt.perl.org/Ticket/Display.html?id=123440>

=item *

C<truncate($filename, $size)> now works for files over 4GB in size.

L<[perl #125347]|https://rt.perl.org/Ticket/Display.html?id=125347>
...
L<[perl #127584]|https://rt.perl.org/Ticket/Display.html?id=127584>
...
more compatible with the originals.
L<[perl #127619]|https://rt.perl.org/Ticket/Display.html?id=127619>

=item *

Fixed the issue where a C<< s///r >>) with B<< -DPERL_NO_COW >> attempts
to modify the source SV, resulting in the program dying.
L<[perl #127635]|https://rt.perl.org/Ticket/Display.html?id=127635>
...
regexes.
L<[perl #127599]|https://rt.perl.org/Ticket/Display.html?id=127599>
...
returned by C<caller()> as negative numbers.
L<[perl #126991]|https://rt.perl.org/Ticket/Display.html?id=126991>

=item *

C<< unless ( I<assignment> ) >> now properly warns when syntax
warnings are enabled.
L<[perl #127122]|https://rt.perl.org/Ticket/Display.html?id=127122>
...
C<isaelem> magic from the elements.
L<[perl #127351]|https://rt.perl.org/Ticket/Display.html?id=127351>
...
handler as well as it's normal value, messing up the stack.
L<[perl #126621]|https://rt.perl.org/Ticket/Display.html?id=126621>


=item *

Fixed an operator precedence problem when C< castflags & 2> is true.
L<[perl #127474]|https://rt.perl.org/Ticket/Display.html?id=127474>
...
and correctly sets C<$AUTOLOAD> too.
L<[perl #124387]|https://rt.perl.org/Ticket/Display.html?id=124387>
L<[perl #127494]|https://rt.perl.org/Ticket/Display.html?id=127494>
...
L<[perl #8904]|https://rt.perl.org/Ticket/Display.html?id=8904>
...
instead of the proper error message.  This has now been fixed.
...
of throwing an error. This is now fixed.
...
filename of the form F<GLOB(0xXXXXXXXX)>.
L<[perl #125115]|https://rt.perl.org/Ticket/Display.html?id=125115>
...
modules.
L<[perl #124181]|https://rt.perl.org/Ticket/Display.html?id=124181>
...
=head1 Known Problems fixed elsewhere

Without cperl there are many known remaining security problems.

=over

=item *

C<'.'> in C<@INC> can lead to easy remote attacks. Use C<-Dfortify_inc> or
cperl instead.
...
Hash security: C<PERL_HASH_SEED_DEBUG> can trivially expose the hash
seed and function via C<perl -V>. Use cperl instead.

=item *

Hash keys are never tainted. Use cperl instead.

=item *

Datastructures bigger than I32 or 2G - strings, hashes and arrays -
cannot deal with an index > I32 or 2GB, which can be exploited if the
attacker is able to create such big datastructures.

Hek lengths, used for symbol and package names are internally limited
to I32, but accept strings which can be up to 2GB (not 4GB) long.

Hashes with more than I32 keys cannot be iterated over.

Array keys are limited to int, not SSize_t.

syswrite cannot write strings > 2GB.

See 5484e6b881b0d997d55e in cperl for the fixes, which harmonizes and
limits array and hash indices to SSize_t (max 2Gb), and string lengths
to STRLEN (4Gb).
...
Unicode TR39 security with the 2 HANGUL FILLER chars usable as perl ids.

cperl removed the perl XIDS and XIDC properties from the two HANGUL FILLER
characters B<+UFFA0> and B<+U3164>.  This is in deviation from Unicode 1.1.

See L<https://github.com/jagracey/Awesome-Unicode#user-content-variable-identifiers-can-effectively-include-whitespace>,
L<http://www.unicode.org/reports/tr39/> and esp.
L<http://www.unicode.org/L2/L2006/06310-hangul-decompose9.pdf>
...
Use cperl instead, see L<[cperl #166]|https://github.com/perl11/cperl/issues/166>
...
See L<[cperl #171]|https://github.com/perl11/cperl/issues/171>

Use cperl instead.

=item *

L<[perl #128252]|https://rt.perl.org/Ticket/Display.html?id=125341>,
Assert fail in op.c with something like C<sub{\@0[0]=0>.

Not fixed in 5.24.1, only in cperl and 5.26.

=item *

L<[perl #128182]|https://rt.perl.org/Ticket/Display.html?id=128182>,
Crash with C<require $nonstring>

If something other than a plain string (e.g. a reference or typeglob)
whose stringified form contains a null character is passed to C<require()>
or C<do()>, it crashes, as of v5.19.3-130-gc8028aa, because the code in
...
L<[perl #128171]|https://rt.perl.org/Ticket/Display.html?id=128171>, 
assert fail with C<< /@0{0*->@*/*0 >>

If a syntax error such as C<*> (multiply) followed by an arrow causes the
...
L<[perl #128106]|https://rt.perl.org/Ticket/Display.html?id=128106>
Crashing reset with non-globs

C<reset> with a string argument assume that anything in a
...
L<[perl #127952]|https://rt.perl.org/Ticket/Display.html?id=127952>
Misoptimization for negated constant-ish on lhs of logop

Negations are being incorrectly deleted from the op tree for an OP_AND or
...
This is a very rare situation: it requires the lhs to be an OP_NOT
...
L<[perl #128508]|https://rt.perl.org/Ticket/Display.html?id=128508>
Wrong line numbers with perl -x

Not fixed in 5.24.1, only in cperl and 5.26.

=item *

h2ph: Missing %sizeof for some primitive types

This bug exists since 5.6.1 (2003), even if the fix is trivial.
L<https://bugs.debian.org/cgi-bin/bugreport.cgi?bug=190887>

This is a problem e.g. with incorrect ioctl values from ioctl.ph:
C<TIOCSPTLCK>, C<TIOCGPTN>

Not fixed in 5.24.1, only in cperl.

=item *


L<[perl #128238]|https://rt.perl.org/Ticket/Display.html?id=128238>
Crash with non-stash in stash, e.g. C< %: = 0 >

When searching a stash for substashes, it assumed anything ending in
C<:> was a substash, whereas substashes end in <::>. So check for a
double colon before recursing.

But even if the name of the stash entry ends with C<::>, it may not itself
contain a real stash (though this only happens with code that assigns
directly to stash entries, which has undefined behaviour according to
perlmod), so skip hashes that are not stashes.
...
L<[perl #128086]|https://rt.perl.org/Ticket/Display.html?id=128086>
Wrong precedence in C<hv_ename_delete>
...
uninit warning from C<$h{\const}> crashes

The code that prints the the name and subscript of a hash element
in an "uninitialized variable" warning assumes that a constant
...
L<[perl #128316]|https://rt.perl.org/Ticket/Display.html?id=128316>
Preserve errno from failed system calls

Not fixed in 5.24.1, only in cperl and 5.26.

=item *

Handle get magic with globs in the regex compiler.  Correctly restore
context, esp. when loading unicode swashes [perl #76538], and
esp. globs still need to be protected from side-effects, even if
doesnot crash anymore since 5.14.

The following little test program shows the problem:
...
Only fixed in suse perl and cperl.

=item *

regexp backref overflow crash with many groups (>I32)

Only fixed in suse perl and cperl.

=item *

Undefined overflow in C<S_sv_2iuv_common()>

Only fixed in suse perl since 5.18 and cperl since 5.24.

=item *


L<[perl #128313]|https://rt.perl.org/Ticket/Display.html?id=128313>
Leak in perl 5.24 with strict and regex posix char classes.
...
L<[perl #128204]|https://rt.perl.org/Ticket/Display.html?id=128204>
Crash with C<@a &.=> etc.

The new bitwise operators in their assignment forms are not correctly
...
Scalar-List-Utils: set_subname cannot handle utf8 nor binary names.

Only fixed in cperl. And with Sub::Name on cpan with a different signature.

=item *

Wrong utf8 handling in L<Net::Cmd>.

L<https://bugzilla.opensuse.org/show_bug.cgi?id=493978>

OTRS pop3 support chokes when sucking UNICODE mail from a POP3 (Exchange
2003) server with the following error:
"Wide character in syswrite at /usr/lib/perl5/5.10.0/Net/Cmd.pm line 232,
 <PRODUCT> line 4."

See the following OTRS list discussion:
L<http://lists.otrs.org/pipermail/otrs/2009-April/026146.html>

Only fixed in suse perl and cperl.

Note that libnet has hundreds of more open tickets:
L<https://rt.cpan.org/Dist/Display.html?Name=libnet>
