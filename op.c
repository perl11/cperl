...
/* Some typestashes were corrupted upstream somehow. */
#if PTRSIZE == 8
...
Perl_class_role_finalize(pTHX_ OP* o)
{
    ...
    for (i=0; *fields; i++) {
        ...
        cv = newSUB(floor, newSVOP(OP_CONST, 0, newSVpvn_flags(key, klen, PadnameUTF8(pn))),
                    NULL, op_clone_optree(body, TRUE));
