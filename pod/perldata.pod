...
unicode confusable and spoofing security problems.  However,
dynamically created symbols via string refs are not normalized.
C<${"$decomposed"}> stays decomposed.  This is similar to Python 3,
but Python 3 normalizes to the non-canonical NFKC.  See
