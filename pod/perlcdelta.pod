...
Assignment type violations are now warned, with C<use warnings 'types'>
enabled. Only signature type violations or C<use types 'strict'> violations
are fatal.
...
Note that p5p doesn't think that this is a security issue.
...
the maximal number of nested hash or arrays to 3000. Cpanel::JSON::XS
has it at 512.
