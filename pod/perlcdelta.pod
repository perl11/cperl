...
=head2 Storable stack overflow or exit

By reading malcrafted local Storable files you could easily overwrite
the local stack with controlled data.  With bigger values you could
cause an immediate exit, without backtrace or an exception being
caught.

Note that p5p doesn't think that local Storable files are a security issue.
...
cperl only. in perl5 announced for 5.26.
