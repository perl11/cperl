...
B<Indirect Object Syntax resolves the syntax dynamically, which used to confuse
the perl5 porters. See below for more details.>
...
Beware that this syntax is dynamically typed and thereby fragile.

C<save $file> is better to read but can be confusing. It's not clear
if C<save> is a method provided by the C<File> class or simply a
...
allows subroutine names to be written as barewords, Perl has to check
...
To parse this code, Perl uses the dynamic approach based on what
package names it has seen, what subroutines exist in the current
package, what barewords it has previously seen, and other
input. Dynamic types can produce very surprising results if the load
order changes.
