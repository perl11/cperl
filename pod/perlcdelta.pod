...
All stored names, identifiers and literals are stored as normalized
NFC unicode, which prevents from TR39 and TR36 unicode confusable and
spoofing security problems.

Note that even perl6 stores different names for confusables, which
match each other due to their NFG rules.
