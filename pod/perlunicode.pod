...
identifier names). See L<utf8>.

=item C<Unicode identifiers follow the Moderately Restrictive Level>

perl5 allows unsafe and confusable unicode identifiers, and even
allows NUL in identifiers.

cperl on the other hand follows the B<Moderately Restrictive Level>
unicode security standard for identifiers.  I.e. All characters in
each identifier must be from a single script, or from any of the
...
Mixed scripts outside of the Moderately Restrictive Level must be
explictly declared via C<use utf8 scriptnames...;>.

All unicode characters in identifiers are normalized to NFC, similar
to Python 3, so spoofing and confusable attacks are much harder.

See L<utf8/Valid scripts> and L<perldata/Identifier parsing> or L<perlsec/Unicode>
...
the appropriate Unicode encoding. The utf8 and unicode hints are set for
compile-time.
(C<BOM>-less UTF-8 cannot be effectively recognized or differentiated
from ISO 8859-1 or other eight-bit encodings.)

UTF-32 BOM's and scripts cannot be handled yet.
...
There is currently nothing in Perl 5 that ignores the differences in
strings. Only identifiers, such as all names are normalized to NFC (in
cperl only). You'll have to specially handle it in strings.  The
usual advice is to convert your inputs to C<NFD> before processing
further.
...
Starting in cperl v5.26c and Perl 5.28.0, the C<split> function with a
pattern specified as a string containing a single space handles
whitespace characters consistently within the scope of of
C<unicode_strings>. Prior to that, or outside its scope, characters
that are whitespace according to Unicode rules but not according to
ASCII rules were treated as field contents rather than field
separators when they appear in byte-encoded strings.
...
perl5 has a broken range operator for utf8 encoded strings, only cperl
has this fixed.  When the right side of the range is a UTF-8 encoded
string, but the left side not, downgrade the right side to native
octets.  E.g.
