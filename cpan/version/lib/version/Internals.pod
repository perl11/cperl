...
A version which only works with cperl, as in C<use cperl> or C<$Config{usecperl}>
is defined with a "c" suffix to a regular version number.
External version parsers which scan F<.pm> files does not understand this, but the
internal version parser does.
...
An optional hash value of 1 denoting a 'c' suffix.
