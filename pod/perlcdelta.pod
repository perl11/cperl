...
This is a new syntax warning, which is deprecated, and will disappear with v5.30c.
Hash slices autovivified in for loops lists and as subroutine
arguments. This was a bug, but to check all your code for correctness and portability
we keep this warning for 2 releases.
See L<[cperl #347]|https://github.com/perl11/cperl/issues/347> and
...
Fixed two testcases where autovivifying hash slices were actually used.

    ++$_ for @INC{"charnames.pm","_charnames.pm"}
=>
    $INC{"charnames.pm"}++; $INC{"_charnames.pm"}++;
...
Fixed autovivifaction of hash slices in C<for> loops and sub args.
