...
and for cperl:
L<https://scan.coverity.com/projects/perl11-cperl>
...
To get valgrind and for more information see L<http://valgrind.org/>
...
Install L<http://lcamtuf.coredump.cx/afl/>
and read L<https://medium.com/@dgryski/fuzzing-perl-xs-modules-with-afl-4bfc2335dd90>.

On darwin and Windows recommended is the llvm_mode C<CC=afl-clang-fast> variant.
On MacPorts you need to build with
C<LLVM_CONFIG=llvm-config-mp-xx CC=clang-mp-xx CXX=clang++-mp-xx>
and compile with C<AFL_CC=clang-mp-xx>.
With llvm be sure to use the very same C<AFL_CC> settings.
On linux/freebsd it's still about 20% faster due to a faster C<fork()>.

afl-fuzz creates variations of src files in the C<-i> directory as input.

My sample build cmdline for macports is this. On linux just leave out the C<AFL_CC>.
...
Fuzzing with L<asan|/AddressSanitizer> or C<DEBUGGING> is also
possible, but very slow.

For testing an XS library and avoid the C<fork()> overhead, check the
C<afl_persistent_loop> as described in the medium blog post and
L<https://lcamtuf.blogspot.de/2015/06/new-in-afl-persistent-mode.html>

=head2 LibFuzzer

See L<http://llvm.org/docs/LibFuzzer.html> with C<-Accflags=-fsanitize=fuzzer>,
esp. since clang-6.0.

Contrary to afl-fuzz you need a compiled-in fuzz target C<LLVMFuzzerTestOneInput>.
...
latest GNU CC manual at
L<http://gcc.gnu.org/onlinedocs/gcc/Gcov-Intro.html#Gcov-Intro>

=head1 COMPILER OPTIMIZATIONS

cperl's default is C<-O3> which includes inlining, perl5's default is
C<-O2>.  The most basic optimizations are C<-march=native> (enabling
CPU specific optimizations and auto-vectorization) and C<-flto> (link
time optimizations).

=head2 LTO

or "Link Time Optimization" via C<gcc -flto=4> or C<clang -flto=thin>.
This requires the C<gold> linker, and enables multi-threaded
...
Without C<-DDEBUGGING> F<Configure> probes to set the best C<-flto> flags
automatically.

=head2 PGO

or "Profile Guided Optimization". This is mostly used on non-linux
system without F<perf>, and works with most compilers: ICC, GCC,
clang, Oracle Solaris Studio, MSVC, IBM XL C/C++.

It is not using a sampling profiler, but an instrumenting profiler, with
some minor run-time overhead.

First you need to compile your binaries with enabled profiling, on gcc
and most other compilers with C<-fprofile-generate> which generates a
F<.gcno> file for each object file. (The same file that is used for
gcov coverage reports).  Then you must run a few tests, which records
coverage data into F<.gcda> files.  Then recompile with
C<-fprofile-use>: it will compile with the gathered F<.gcda> coverage
data and infer if an branch is LIKELY or UNLIKELY.
On L<llvm|http://clang.llvm.org/docs/UsersManual.html#profiling-with-instrumentation>
it is creating a F<default.profraw> file, which must be converted via
C<llvm-profdata> to a F<default.profdata> file.

You can use the new F<miniperl.pgo> and F<perl.pgo> targets to create
the profile guided optimized binaries.

=head2 AutoFDO

L<AutoFDO|https://gcc.gnu.org/wiki/AutoFDO> can only be used on linux
systems with F<perf>, a sampling profiler. The advantage is that you
build the fully optimized binaries, run a few practical tests through
C<perf> with the L<sampling
profiler|http://clang.llvm.org/docs/UsersManual.html#using-sampling-profilers>,
which collect statistics using hardware performance counters, which
will then guide the autofdo optimizer (via C<-fauto-profile>) to
re-link the binaries. Not all branches will be profiled, but the
overall accuracy is generally better.

On most recent kernels you need to recompile
L<autofdo|https://github.com/google/autofdo> by yourself, if you get a
missing type 79 error.
...
The perl/cperl Makefile already contains two targets for autofdo on linux:
C<miniperl.autofdo> and C<perl.autofdo>.

Both profiling targets, the sampling autofdo and the instrumented pgo,
use customizable benchmark scripts F<minibench.sh> and
F<autofdobench.sh>, which can be tuned to your specific workload to be
optimized. Per default it only runs some basic op tests.

Note: Support for a shared F<libcperl.so> and clang came only with
cperl 5.29.1c.  Before you need to profile it by hand.

=head2 prelink

L<prelink|https://community.linuxmint.com/tutorial/view/473> from
libreoffice is a post-link optimizer.  It fixes up startup
relocations, but with PIE, ASLR and constantly updated libraries
it's a bit of a L<hassle|https://lwn.net/Articles/341244/> and is not
recommended, esp. not on 32bit systems.

=head2 BOLT

L<llvm-bolt|https://github.com/facebookincubator/BOLT> is also
post-link optimizer, but mouch more powerful. It analyzes F<perf>
traces from a running executable or service, and then rewrites the
binary to be faster, similar to L</AutoFDO>.

I've compiled bolt at F</usr/src/llvm/llvm-bolt/build>, added the bin
path to my PATH C<export PATH=/usr/src/llvm/llvm-bolt/build/bin:$PATH>,
added the needed C<-Wl,-q> flag to C<ldflags> and C<lddlflags>:

    sed -i -e"s/ldflags='/ldflags='-Wl,-q /" config.sh
    sed -i -e"s/lddlflags='/lddlflags='-Wl,-q /" config.sh

Sorry, C<-Alddlflags> is still unusable.

Compile F<miniperl> and use F<minibench.sh> from L</AutoFDO> to create
the profile.
...
Which leads to an improvement 9% on the already link-time optimized
executable. Note that the generated F<bolt.fdata> is readable and can
...
Useful C<.gdbinit> settings and macros are:
...
Now the abbrevations C<od> and C<sd> can be used, like C<od PL_op> or C<sd sv>.

=head2 PERL_SV_DEBUG_FIELDS

With C<-DEBUGGING> several SV_HEAD fields are also available as unions
of XPV* pointers and bitfields, to easily see the SvANY values and type
and flags in an interactive debugging session.

Without C<-DEBUGGING> you may enable it with C<-Accflags=-DPERL_SV_DEBUG_FIELDS>
or you may disable it with C<-Accflags=-DPERL_SV_DEBUG_FIELDS=0>.
...
To do that simply edit F<~/.ddd/init> file and add after:
...
so now you can do C<ivx> and C<pvx> lookups or you can plug there the C<sv_peek>
"conversion":

  Perl_sv_peek(my_perl, (SV*)()) // sv_peek

(The C<my_perl> is for threaded builds.)  Just remember that every line,
but the last one, should end with C<\n\>
...
If you see in a debugger a memory area mysteriously full of C<0xABABABAB>
or C<0xEFEFEFEF>, you may be seeing the effect of the C<Poison()> macros, see
L<perlclib>.
...
which will expand the macros using C<cpp>.  Don't be scared by the
results.
