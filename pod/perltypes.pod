...
=head2 Static types in lexical variables

Perl optionally allows static types, i.e. an existing package
name, declared with lexical variables. Lexicals are stored in
...
type optimizations. You must assign a value within the declaration.
  ...
  my ($d:const, $free) = (0,1);
  my (int $i:const, string $s:const) = (0,"1");
...
Accesses to const hashes can be optimized via L<study> at run-time.
...
declare or allow declaration for during execution of L<perlsub/CHECK>
blocks.
