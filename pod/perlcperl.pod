...
for C<%>; using C<i_modulo> which is different than the generic
C<modulo> operator.
...
cperl with typed or constanst integers behaves like perl5 with use integer.
To fall back to the old behavior use untyped variables and no integer.
...
Rationale: This is a bad perl5 design. cperl uses proper type dispatch
and with both integer arguments it promotes the modulo op to i_modulo,
which behaves differently (or "normally").
Using extra logic to prevent using the libc implementation also makes
perl5 C<"%"> slower.
...
associations, such as C<tie>.  With a jit only these will benefit from
a jit.
...
cperl adds many more traditional compile-time optimizations: type
optimizations, shaped arrays, usage of constants, loop unrolling,
omit unnecessary bounds checks, function inlining, and the change of
compile-time static method calls, determined either by name or by type
and const-ness of all searched packages to static function calls.
Thus the dynamic method search, in which object this method is
implemented is avoided. This is about ~10% faster.
