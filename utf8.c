                           ...
                           const bool consider_overlongs)
{
     ...
     * the initial bytes of the representation of a code point, but possibly
...
#define SWASH_INIT_RETURN(x)   \
    ...
    assert(listsv != UNDEF || strNE(name, ""));
...
/* NFC normalization from Unicode::Normalize */
...
/* Regenerated by Unicode-Normalize mkheader -ind */
#include "un8ifcmb.h"
#include "un8ifcan.h"
#include "un8ifcmp.h"
#include "un8ifexc.h"
...
static int S_bsearch_exc (const void *ptr1, const void *ptr2) {
...
static int S_dec_canonical(U8 *d, UV uv)
{
	...
	return 0;
# if 1
    return plane[(uv >> 8) & 0xff];
# else    
    row = plane[(uv >> 8) & 0xff];
    if (row) {
        strcpy(d, row[uv & 0xff]);
        return strlen(d);
    } else {
        return 0;
    }
    ...
    if (row) {
        ...
        if (!vi)
            ...
            e = (UN8IF_canon_exc_t *)bsearch(&uv, &UN8IF_canon_exc,
                    UN8IF_canon_exc_size, sizeof(UN8IF_canon_exc[0]), S_bsearch_exc);
            if (e) {
                int l = strlen(e->v);
                memcpy(d, e->v, l+1); /* incl \0 */
                return l;
            }
        }
        else {
...
static U32 S_composite_uv(UV uv, UV uv2)
{
    const UN8IF_complist_s ***plane, **row, *cell;

    if (UNLIKELY(!uv2 || (PERL_UNICODE_MAX < uv) || (PERL_UNICODE_MAX < uv2)))
	return 0;

    if (Hangul_IsL(uv) && Hangul_IsV(uv2)) {
	U32 lindex = uv  - Hangul_LBase;
	U32 vindex = uv2 - Hangul_VBase;
...
#define Renew_d_if_not_enough_to(need)          \
    STRLEN curlen = d - dstart;                 \
    if (dlen < curlen + (need)) {               \
        dlen += (need);                         \
...
S_pv_utf8_decompose(pTHX_ U8* s, STRLEN slen, U8** dp, STRLEN dlen)
{
    ...
    while (p < e) {
	STRLEN retlen;
	UV uv = utf8n_to_uvchr(p, e - p, &retlen, UTF8_ALLOW_ANY);
	if (!retlen)
...
S_pv_utf8_reorder(pTHX_ U8* s, STRLEN slen, U8** dp, STRLEN dlen)
{
    ...
    UN8IF_cc* seq_ptr = seq_ary; /* use array at the beginning */
    UN8IF_cc* seq_ext = NULL; /* extend if need */
	...
	UV uv = utf8n_to_uvchr(p, e - p, &retlen, UTF8_ALLOW_ANY);
	if (!retlen)
            /* diag_listed_as: Malformed UTF-8 character in reorder (empty string) */
	...
	if (curCC != 0) {
		...
		if (CC_SEQ_SIZE == cc_pos) { /* seq_ary full */
		    STRLEN i;
		    Newx(seq_ext, seq_max, UN8IF_cc);
		    for (i = 0; i < cc_pos; i++)
			seq_ext[i] = seq_ary[i];
		}
		else {
		    Renew(seq_ext, seq_max, UN8IF_cc);
		}
	...
	if (cc_pos) {
		...
		qsort((void*)seq_ptr, cc_pos, sizeof(UN8IF_cc), S_compare_cc);
...
S_pv_utf8_compose(pTHX_ U8* s, STRLEN slen, U8** dp, STRLEN dlen)
{
	...
	UV uv = utf8n_to_uvchr(p, e - p, &retlen, UTF8_ALLOW_ANY);
	if (!retlen)
            /* diag_listed_as: Malformed UTF-8 character in compose (empty string) */
	...
	if (!valid_uvS) {
	    ...
	    bool composed;
	    /* not blocked:
		 iscontig (FALSE) && cc_pos == 0      -- contiguous combination
		 curCC == 0 && preCC == 0     -- starter + starter
		 curCC != 0 && preCC < curCC  -- lower CC */
            /* try composition */
            U32 uvComp = S_composite_uv(uvS, uv);
...
char *
Perl_pv_uni_normalize(pTHX_ char *s1, STRLEN slen, STRLEN *dlenp)
{
    U8 *s, *d, *send, *dend;
    STRLEN dlen;
#ifdef DEBUGGING
