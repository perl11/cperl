...
chdir 't' if -d 't';
require './test.pl';
note("HvFILL not cached anymore") if $] >= 5.025003;
plan(tests => 6);
...
sub about_as_fast_ok {
    ...
    if ($name =~ /^TODO /) {
        $::TODO = "no cached HvFILL" if $ratio >= 0.25;
        $name =~ s/^TODO //;
    }
    cmp_ok( $ratio, '<', 0.25, "<25% faster, $name" );
    diag( sprintf "%20s: %12.2f/s\n", $key1, $iters_per_second{ $key1 } );
    diag( sprintf "%20s: %12.2f/s\n", $key2, $iters_per_second{ $key2 } );
};

about_as_fast_ok( $res, 'scalar_big', 'scalar_small',
    "TODO Checking the count of hash keys in an empty hash (global)");

about_as_fast_ok( $res, 'big', 'small',
    "TODO Checking the list of hash keys in an empty hash (global)");

about_as_fast_ok( $res, 'lex_scalar_big', 'lex_scalar_small',
    "TODO Checking the count of hash keys in an empty hash (lexical)");

about_as_fast_ok( $res, 'lex_big', 'lex_small',
    "TODO Checking the list of hash keys in an empty hash (lexical)");

about_as_fast_ok( $res, 'lex_scalar_big', 'scalar_big',
    "TODO Checking the count of hash keys in an empty hash, global vs. lexical");

about_as_fast_ok( $res, 'lex_big', 'big',
    "TODO Checking the list of hash keys in an empty hash, global vs. lexical");
