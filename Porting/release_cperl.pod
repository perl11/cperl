...
or:
    ...
    make -s -j4 test && make -s install DESTDIR=`pwd`/../cperl-5.30.0-inst

    mkdir ../cperl-5.30.0
    cp -rp .git ../cperl-5.30.0/
    cd ../cperl-5.30.0
    git reset --hard
    git clean -dxf
    perl Porting/makerel -x  # src tarball

    for t in deb rpm osxpkg solaris pkgin; do
      fpm -s dir -t $t -C cperl -p ../cperl-5.30.0-inst/ \
        --version 5.30.0 \
        -m rurban@cpan.org \
        --url http://perl11.github.io/cperl/$t/ \
...
F<Porting/do-make-srctarball> contains this:
    ...
    mkdir ../cperl-5.30.0-build
    cp -rp .git ../cperl-5.30.0-build/
    cd ../cperl-5.30.0-build
    git reset --hard
    git clean -dxf
...
.rpm, osx .pkg, bsd and solaris packages, but not yet windows .msi
    ...
    fpm -s dir -t deb -n cperl -v 5.30.0-1 -m 'rurban@cpan.org' \
        -d 'zlib1g (>= 1:1.2.2.3)' -d libbz2-1.0 -d 'libgdbm3 (>= 1.8.3)' \
        -d 'libdb-5.1' \
        --url http://perl11.github.io/cperl/deb/ \
        --deb-changelog pod/perl5222cdelta.pod \
        -C ../cperl-5.30.0-inst .

    # on i386 and x86_64 centos6 and a centos7 machine. no -1 suffix
    fpm -s dir -t rpm -n cperl -v 5.30.0 -m 'rurban@cpan.org' \
        -d 'zlib' -d bzip2-libs -d 'gdbm' -d 'db4' \
        --url http://perl11.github.io/cperl/rpm/ \
        -C ../cperl-5.30.0-inst .

    # on an amd64 osx machine
    fpm -s dir -t osxpkg -n cperl -v 5.30.0 -m 'rurban@cpan.org' \
        --url http://perl11.github.io/cperl/osxpkg/ \
        -C ../cperl-5.30.0-inst .

    # on i386 and amd64 solaris machines
    fpm -s dir -t solaris -n cperl -v 5.30.0 -m 'rurban@cpan.org' \
        --url http://perl11.github.io/cperl/solaris/ \
        -C ../cperl-5.30.0-inst .

    # on i386 and amd64 bsd machines
    fpm -s dir -t pkgin -n cperl -v 5.30.0 -m 'rurban@cpan.org' \
        --url http://perl11.github.io/cperl/pkgin/ \
        -C ../cperl-5.30.0-inst .
...
=head2 prepare the repos on perl11.github.io

You need write permissions to the F<perl11/perl11.github.io> repo on
...
use our static L<http://perl11.github.io/cperl/> website on github for now.

    cd ..
    git clone git@github.com:perl11/perl11.github.io.git
...
Add the debs to the pool:
    ...
    aptly repo add cperl ../../cperl_5.30.0-1_{i386,amd64}.deb
    aptly publish repo cperl
    rsync -av ~/.aptly/public/* perl11.github.io/deb/
...
perl11.github.io/cperl/STATUS.html and the pods.

From markdown and pod:

    ROOT=../perl11.github.io/cperl
