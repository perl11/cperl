...
CIRCUMFLEX ACCENT) followed by any one of the characters
C<[][A-Z^_?\]>.  These names are all reserved for special uses by
Perl; for example, the all-digits names are used to hold data captured
by backreferences after a regular expression match.
...
Its purpose is to allow your non-ASCII Perl scripts to not have to be
written in UTF-8. But if you need something like this functionality,
you should use the L<encoding> pragma, which has fewer nasty side
effects and allows lexically scoped source encodings.
...
This variable was added in Perl 5.8.2. perl 5.26.0 removed it, but
cperl kept it.
