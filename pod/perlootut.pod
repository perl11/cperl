...
L<Mouse> is the fast, smaller variant of C<Moose> and the currently
recommended OO system for perl5.

Moose has a large compile-time penalty. Though significant progress
...
essential, you may not be able to use Moose (we recommend that you
...
Compatibility with Moose has been the utmost concern. The sugary interface
is highly compatible with Moose. Even the error messages are taken from
Moose. The Mouse code just runs its test suite 4x faster.
