...
If 0, then you have specified the C<G_DISCARD> flag.
...
C<G_ARRAY> flag and the mechanics of accessing the returned items from the
Perl stack.
...
context to the Perl subroutine by using either C<G_SCALAR> or C<G_ARRAY>.
...
the C<G_NOARGS> flag, it is still possible for the Perl subroutine that
...
type of event, specify the C<G_EVAL> flag.  It will put an I<eval { }>
...
If C<G_DISCARD> is specified, the return value will always be 0.

=item *

If C<G_ARRAY> is specified I<and> an error has occurred, the return value
will always be 0.

=item *

If C<G_SCALAR> is specified I<and> an error has occurred, the return value
...
See L</Using G_EVAL> for details on using C<G_EVAL>.

=head2 G_KEEPERR

Using the C<G_EVAL> flag described above will always set C<$@>: clearing
...
The C<G_KEEPERR> flag is meant to be used in conjunction with C<G_EVAL> in
...
C<G_EVAL> is not used.

When C<G_KEEPERR> is used, any error in the called code will terminate the
call as usual, and the error will not propagate beyond the call (as usual
for C<G_EVAL>), but it will not go into C<$@>.  Instead the error will be
...
Note that the C<G_KEEPERR> flag does not propagate into inner evals; these
may still set C<$@>.

The C<G_KEEPERR> flag was introduced in Perl version 5.002.
...
We aren't passing any parameters to I<PrintUID> so C<G_NOARGS> can be
specified.

=item 3.

We aren't interested in anything returned from I<PrintUID>, so
C<G_DISCARD> is specified. Even if I<PrintUID> was changed to
return some value(s), having specified C<G_DISCARD> will mean that they
...
Because we specified C<G_DISCARD>, it is not necessary to check the value
...
string (I<$s>) and an integer (I<$n>).  The subroutine will simply
print the first I<$n> characters of the string.
...
as C<SP>, but better with the C<ST(i)> stack access macro.
...
C<PUSHMARK> and C<PUTBACK> macros.  The purpose of these two macros,
in this context, is to count the number of parameters you are pushing.
Whenever Perl is creating the C<@_> array for the subroutine, it fills
it with the elements from MARK to SP, and for XSUB function without
C<@_> array, the C<MARK> pointer marks the beginning of the arguments.

The C<PUSHMARK> macro marks the begin of argument stack pointer with
C<MARK>. Even if you aren't passing any parameters (like the example
shown in the section L</No Parameters, Nothing Returned>) you must
still call the C<PUSHMARK> macro before you can call any of the
I<call_*> functions. Perl still needs to know the arity, the number of
arguments, which is calculated by C<SP - MARK>, and with no arguments
C<MARK> == C<SP>, and C<items> will be 0.
...
Next, we come to C<EXTEND> and C<PUSHs>. This is where the parameters
actually get pushed onto the stack. In this case we are pushing a
string and an integer.

Alternatively you can use the C<XPUSHs()> macro, which combines a
...
The only flag specified this time is C<G_DISCARD>. Because we are passing
...
We wanted list context, so C<G_ARRAY> was used.
...
Specifying the C<G_DISCARD> flag with I<call_*>
...
sequence in the callback (and not, of course, specifying the C<G_DISCARD>
flag).
