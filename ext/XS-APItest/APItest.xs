...
CODE:
#ifdef PERL_SV_DEBUG_FIELDS_DEFINED
    if (STRUCT_OFFSET(SV,sva_u.sv_any) == STRUCT_OFFSET(SV,sva_u.sv_any_dbg._1_5iv)) {
        SV * sv = newSV(0);
        if (!cBOOL(SvTEMP(sv)) && !sv->svf_u.sv_flags_dbg.s_TEMP) {
            sv_2mortal(sv);
            if (cBOOL(SvTEMP(sv)) && sv->svf_u.sv_flags_dbg.s_TEMP){
                RETVAL = TRUE;
                goto end;
...
#else
    RETVAL = TRUE; /* nothing to test, PERL_SV_DEBUG_FIELDS don't exist */
#endif
OUTPUT:
    RETVAL
...
CODE:
	...
	Perl_load_module(aTHX_ flags, SvREFCNT_inc(name), SvREFCNT_inc(ST(2)));
        ...
        Perl_croak(aTHX_ "load_module can't yet support %d items", items);
