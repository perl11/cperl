...
removing one recently added line in core. Blocking L<B::Generate> and
internal optimizations for no technical reasons already did too much
harm.  Discussing this block led to even worse blocks on
optimizations.
...
coretypes can only be built-in. Provide the 3 basic ones, and the
