...
This document describes differences between the 5.22.2 release and the 5.24.0
release.
...
B<cperl note>: Some perl-5.22.2 and perl5.24.0 fixes are broken and
were fixed in cperl-5.22.2 before properly. p5p does not really
understand its codebase and refuses to take our bugreports and proper
fixes, so usage of perl-5.22.2 and perl5.24.0 is B<officially
discouraged>.  The official explanation of p5p is refering to their
new code of conduct, which now disallows criticsm on their broken
development.

See L<perlcdelta> for the changes in the cperl release.
...
glibc) create a file with permissions C<0066>, leaving world read and
...
=head2 Lexical $_ has been removed in perl5

But not in B<cperl>! cperl fixed the usage of lexical $_ in 5.22.2c already.
...
proposed.  The feature has now been removed from perl5 and will fail to compile
there.

perl5 does not follow cperl and ignores its new features and fixes. So
they had no idea about the fix. The problem is that lexical $_ is
required for given/when and smartmatch.
...
Note that the new gcc and clang fast overflow builtins are not used in
perl5, only cperl, which makes normal arithmetic in cperl >20% faster.
...
L<Module::CoreList> has been upgraded from version 5.20151213 to 5.20160628c.
...
The implementation of perl's B<context stack> system, and its internal API,
...
Perl 5.24.0 represents approximately 4 months of development since Perl 5.22.2
