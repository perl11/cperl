...
cperl can parse and run almost all of perl5 code. But there are a few
...
Rationale: Illegal prototypes are parsed as signatures. Illegal
signatures throw a parser error.  The C<'illegalproto'> warning is
only thrown with explicit extra C<:prototype()> declarations.
...
C<@_> is empty when signatures are declared.

Rationale: With signatures copying all values to @_ also leads to
double copying which is 2x slower. @_ is not needed anymore. Either
use no signatures to use @_ variables or use sigantures.
...
Rationale: This clashes with prototypes. A bare C<$> is a prototype
...
generic C<modulo> operator.

With perl5 without use integer it uses a different modulo definition:
...
integer.  The compiler simply promotes C<modulo> to C<i_modulo> if both
arguments are integers at compile-time.  To fall back to the old
behavior use untyped variables and no integer.
...
Rationale: This is a bad perl5 design and a side-effect from using
...
extra logic to prevent using the libc implementation also makes perl5
C<"%"> slower.

=item compile-time constant folding overflows with integer literals

Operations with only literal integers will not overflow to numbers,
similar to perl5 with use integer.
Only C<"+"> will try to use UV (an unsigned value) as result, but not
...
IV (signed integers). Note that even comparison operators are integerized.
    ...
    my $uv_max = 1 + 2 * (~0 >> 1); # i_multiply 2 + IV_MAX will create -1
    =>
    my $uv_max = 1 + 2.0 * (~0 >> 1); # force a number to create UV_MAX
...
All coretests pass, just F<t/op/inc.t> had to fixed, by enforcing a
number like above.

Rationale: This was the original issue 2002 which disabled automatic
integerization of all arithmetic ops with constants. C<"+"> is a good
exception to allow unsigned results (with the new C<u_add> op), but
all other operations, like C<"*"> need to be preciser when inexact
numbers are demanded, and return signed integers.

Exceptions, keeping the old behaviour:
...
improved, and our changes are not incompatible.
...
=item Support default $self invocant with methods
...
=item Add :pure attribute for subroutines
...
We unblocked the possibility to perform dynamic run-time optimizations
of function bodies, the equivalence of C<eval "sub bla { ...}"> by
removing one recently added line in core, which potentially is
problematic with threads and needs to be solved in the threaded
code. Blocking L<B::Generate> and internal optimizations for no
technical reasons already did too much harm.  Discussing this block
led to even worse blocks on optimizations.
...
=head2 Undo support for binary names (maybe)
...
=head2 Undo the double readonly system (maybe)
...
Make the load factor definable, and change the default from 100% to 90%.
...
Our coretypes and its members are readonly.  Provide fast ops variants
for these types to omit type checks and magic calls at run-time.
Scalars declared as C<Int>, C<Uint>, C<Num>, C<Str> cannot hold magic
associations, such as C<tie>.  With a jit mostly these types will benefit.

We also enabled the C<:const> attribute for all data: scalar, arrays,
hash, functions, packages+classes. C<:const> hashes should of course
be perfect, i.e. optimized to constant-time lookup, eliminating hash
collisions.

At compile-time all UNOP's and BINOP's are promoted from the generic
...
=head2 native unboxed types: int, uint, num, str
...
reference counted, and cannot be used across function calls. They are
only safe to use within certain op sequences, and those optimizations
...
For the FFI we will also provide FFI-specific native types, like
int32, int64, uint32, uint64 and ptr.
...
array indices, ranges and C<str> on hash keys, but has to give up on magic,
...
Status: User code in pure perl or XS is currently not typed-checked nor
inferred, only internal code.
...
arithmetic unops and binops.  F<opnames.h> stores C<PL_op_type_variants>,
all possible type promotions for each op.
...
stopped on each function call border.
...
method call is optimized to a normal functions call.
...
cannot be changed.  All values are pre-initialized. With known indices
the compiler can omit bounds checks on array accesses.
...
stack, because the stack is not garbage collected, only refcounted.
...
besides C<defined-or> C<//=>, which I count as trivial. Even if the
...
bindings as refalias, and many more.  Additionallly most bugs don't
get fixed.

With cperl we do fix bugs, implement features and encourage
discussions.  The lead should be the most competent, not the least
competent, loudest or most likeable one "community person".  We do
encourage criticism on bad design and implementation.  We stay civil
and don't call critics I<"jerks"> or I<"assholes">, unlike in the p5p
community.  We practice software development best practices, we
develop in branches only, and only if the branch passes the test suite
on all major platforms it can be merged.  Pushing directly to master
is forbidden, but we do have to rebase frequently all our branches.
Use the provided rebase helper scripts F<cp-*>. We do use build
servers and performance regression tests.  We follow the perl6
development principles.  We take github pull request and do not ignore
them.

We track stable upstream releases for our releases only. The current
stable release is based in 5.22.1. The previous stable release was
5.14.4, as 5.16 introduced critical instabilities which were not fixed
mostly until 5.20 and this version, and 5.20 alone was not worth to
update cperl to. The previous stable release was 5.8.9 and the one
...
realistic.  So far we have detected 2 grave API mistakes in
5.22.0. But we do merge all official p5p releases into our master
branch monthly.
