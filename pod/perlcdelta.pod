...
C<-Dusedevel> is not needed to be added to all development versions
anymore, it is now the default on such. A big warning is still being
displayed, even in the usual silent auto mode C<-des>.
...
<<<<<<< HEAD
=item mingw
||||||| parent of 962c7a2... Configure: -Dusedevel automatic default
=item *

XXX
=======
>>>>>>> 962c7a2... Configure: -Dusedevel automatic default
