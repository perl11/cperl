...
sub _Perl_Non_Final_Folds {
    ...
    my @retlist = mk_invlist_from_sorted_cp_list(\@is_non_final_fold);
    return \@retlist;
}
...
sub UpperLatin1 {
    my @retlist = mk_invlist_from_sorted_cp_list([ 128 .. 255 ]);
    return \@retlist;
}
...
foreach my $prop (@props) {
    ...
    for my $charset (get_supported_code_pages()) {
        ...
        if ($is_local_sub) {
            my @retlist = eval $lookup_prop;
            die $@ if $@;
            my $invlist_ref = shift @retlist;
            @invlist = @$invlist_ref;
            if (@retlist) {  # If has other values returned , must be an
                             # inversion map
                my $invmap_ref = shift @retlist;
                @invmap = @$invmap_ref;
                $map_format = shift @retlist;
                $map_default = shift @retlist;
            }
        }
        else {
            ...
            if (! @invlist) {
                ...
                if (! $list_ref) {
                    ...
                    warn "Could not find inversion list for '$lookup_prop'";
                    last;
                }
                else {
                    @invlist = @$list_ref;
...
print $keywords_fh MinimalPerfectHash::make_algo($second_level, $seed1,
    $length_all_keys, $smart_blob, $rows, undef, undef, undef, 'match_uniprop' );
