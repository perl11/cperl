...
# endif
    ...
    {
        char _f[80];
        sprintf(_f, "/tmp/cperl-opprof.%d", (int)PerlProc_getpid());
        PL_opprof_io = PerlIO_open(_f, "a");
    }
#endif
...
perl_parse(pTHXx_ XSINIT_t xsinit, int argc, char **argv, char **env)
{
	...
	ret = STATUS_EXIT;
         ...
         * cperl: on windows exit(256) will not be exit(0). this broke pod\buildtoc
         */
#endif
	break;
    case 3:
...
#ifdef USE_CPERL
	PerlIO_printf(PIO_stdout,
		      "cperl Copyright (c) 2012-2017 cPanel Inc, 2017-2019 Reini Urban\n");
#endif
#ifdef MSDOS
	PerlIO_printf(PIO_stdout,
		...
		*/
		if (lastslash) {
		    ...
		    while ((*lastslash = '\0'), /* Do that, come what may.  */
			    ...
			    && (lastslash =
                                  (char *) my_memrchr(prefix, '/', libsize))))
                    {
			if (lastslash[1] == '\0'
			    || (lastslash[1] == '.'
				&& (lastslash[2] == '/' /* ends "/."  */
