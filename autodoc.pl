...
sub perlio () { # parse pod/perlapio.pod and extract documentation info
    my $file = "pod/perlapio.pod";
    ...
    while (<$fh>) {
        ...
        if (/^=item (.*)/) {
            ...
            $hash->{$1} = $1 unless exists $hash->{$1};
            for (split / /, $_) {
                s/X<(.*?)>/$1/;
                $hash->{$_} = $_ unless exists $hash->{$_};
            }
        }
    }
    return $hash;
...
# deprecated.
my @missing_api = grep $funcflags{$_}{flags} =~ /A/ && $funcflags{$_}{flags} !~ /[MD]/ && !$docs{api}{$_} && !$perlio->{$_}, keys %funcflags;
...
L<perlguts>, L<perlxs>, L<perlxstut>, L<perlintern>, L<perlapio>
