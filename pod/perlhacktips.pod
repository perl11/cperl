...
with C<-fprofile-generate> which generates a F<.gcno> file for each
object file. (The same file that is used for gcov coverage reports).
Then you must run a few tests, which records coverage data into F<.gcda> files.
Then recompile with C<-fprofile-use>: it will compile with the gathered
F<.gcda> coverage data and infer if an branch is LIKELY or UNLIKELY.

=head2 AutoFDO

L<AutoFDO|https://gcc.gnu.org/wiki/AutoFDO> can only be used on linux
systems with F<perf>. The advantage is that you build the fully
optimized binaries, run a few practical tests through C<perf>, which
collect statistics using hardware performance counters, which will then
guide the autofdo optimizer to re-link the binaries.
...
C<miniperl.autofdo> and C<perl.autofdo>. These two targets use customizable
benchmark scripts F<minibench.sh> and F<autofdobench.sh>, which can be tuned
to your specific workload to be optimized.

Note: Support for a shared F<libcperl.so> and clang came only with cperl 5.29.1c.
Before you need to profile it by hand.
