...
=head2 C<eval "q" . chr(overlarge)> buffer overflow

In C<eval "q" . chr(100000000064)> generating the error message
C<Can't find string terminator "XXX"'> was overrunning a buffer
...
A possible divide by zero in string transformation code on Windows needs to be avoided,
fixing a crash when collating an empty string.
...
Perl 5.25.1 represents approximately 4 months of development since Perl 5.25.0
