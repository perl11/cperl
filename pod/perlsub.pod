...
With C<extern> or :C<native> subroutine declarations the types in the
signature are mandatory, and the block is empty. Some non-coretypes,
FFI specific types are imported via the L<ffi> module. The functions
are searched in all loaded shared libraries, a new shared library must
be specified via the C<:native> argument, are loaded beforehand via
C<DynaLoader::dl_load_file()>.  An alternate name for the C function
may be declared via the C<:symbol> attribute.
...
The C<:native> attribute accepts a string for the library name, searched
