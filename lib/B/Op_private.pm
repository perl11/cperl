...
$bits{$_}{4} = 'OPpTARGET_MY' for qw(abs add atan2 chdir chmod chomp chown chr chroot concat cos crypt divide exec exp flock getpgrp getppid getpriority hex i_add i_divide i_modulo i_multiply i_subtract index int int_add int_subtract kill left_shift length link log match mkdir modulo multiply nbit_and nbit_or nbit_xor ncomplement oct ord pow push pushre qr rand rename right_shift rindex rmdir schomp scomplement setpgrp setpriority sin sleep sqrt srand str_concat str_length stringify subst subtract symlink system time trans transr uint_lshift uint_rshift unlink unshift utime wait waitpid);
...
@{$bits{msgrcvl}}{3,2,1,0} = ($bf[3], $bf[3], $bf[3], $bf[3]);
...
@{$bits{sockpairl}}{3,2,1,0} = ($bf[3], $bf[3], $bf[3], $bf[3]);
...
@{$bits{str_concat}}{1,0} = ($bf[1], $bf[1]);
$bits{str_delete}{0} = $bf[0];
$bits{str_exists}{0} = $bf[0];
@{$bits{str_helem}}{1,0} = ($bf[1], $bf[1]);
$bits{str_length}{0} = $bf[0];
...
our %ops_using = (
    ...
    OPpTARGET_MY             => [qw(abs add atan2 chdir chmod chomp chown chr chroot concat cos crypt divide exec exp flock getpgrp getppid getpriority hex i_add i_divide i_modulo i_multiply i_subtract index int int_add int_subtract kill left_shift length link log match mkdir modulo multiply nbit_and nbit_or nbit_xor ncomplement oct ord pow push pushre qr rand rename right_shift rindex rmdir schomp scomplement setpgrp setpriority sin sleep sqrt srand str_concat str_length stringify subst subtract symlink system time trans transr uint_lshift uint_rshift unlink unshift utime wait waitpid)],
