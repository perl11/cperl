...
i.e. Net::DNS.  Perl6 uses types all over.  Every perl5 value is typed
at run-time. Just all the normal ops are generic and are allowed to
change the type of its arguments and its result at will.
...
Without types there will be no multi-dispatch, no proper object
system, and smartmatch is almost impossible without types. There's no
