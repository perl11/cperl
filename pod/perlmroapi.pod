...
Since cperl-5.28 C3 is now the default mro, the old C<dfs> is
implemented in F<ext/mro>.
...
For examples of MRO implementations, see C<S_mro_get_linear_isa_dfs()>
and the C<BOOT:> section of F<ext/mro/mro.xs>, and
C<S_mro_get_linear_isa_c3()> in F<mro_core.c>

Only two mro implementations are known to be useful: The old
dfs (used in perl5). And the new c3, which is the default in python,
perl6 and cperl.

=head1 SEE ALSO

L<https://en.wikipedia.org/wiki/C3_linearization>
