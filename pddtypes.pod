...
A C<my (TYPE EXPR, ...)> declaration should not be mixed with a function call,
i.e. subroutine entry.  C<my> is a reserved keyword and cannot be the name of
a subroutine.

The error should be the same as with
  perl -e'my j $a;'
  => No such class j at -e line 1, near "my j"
and
  perl -e'$i::x; my i $a;'
parses correctly.
...
L<http://github.com/rurban/perl/branch/typed/ro> starts with support for const
...
=head2 assign to const

    $ ./perl -Ilib -MO=Concise -e'my const $a=0; $a=1'
...
The parser detects my const, and marks it with ival=2 for my_attrs_lex.
It creates SASSIGN/AASSIGN with OPf_SPECIAL ('*'),
with left pad* OPpPAD_CONSTINIT and the data as SvREADONLY.

newASSIGNOP throws a compile-time error when the left pad is SvREADONLY,
unless it is CONSTINIT.

SvREADONLY is bypassed in pp_*assign with OPf_SPECIAL.

AASSIGN (multiple assignments) additionally sets OPpASSIGN_CONSTINIT
for the current assignment only, cleaned afterwards.

The subsequent const PAD ops have no CONST marker, so do it in a
new ck_pad() to set OPpPAD_CONST in all readonly pads, so the optimizer
can treat PADops with OPpPAD_CONST as the CONST op. (or replace the PADop
by CONST)
...
op:
  CONST IV=1
  PADSV    OPpPAD_CONSTINIT, targ -> READONLY
...
op:
  ...
  CONST IV=1
  pushmark
  PADAV    OPpPAD_CONSTINIT, targ -> READONLY
  AASSIGN  OPf_SPECIAL
