...
unicode rules for identifiers and literals with mixed script properties:

=over

=item *

The 'Common' and 'Latin' scripts are always allowed and don't need to
be declared.

=item *

The first non-Latin and not-Common unicode script for an identifier is
the only allowed one. Others lead to parsers errors.

=item *

Additional unicode scripts can and should be declared via B<use utf8
'Greek', 'script-name2'...>. This allows mixed scripts in
...
This holds for all identifiers (all names: package, gv, sub,
...
So PERL_HASH_SEED_DEBUG=1 will now hide the seed value in
...
fixed offset to the first argument of every OP, at my_perl via unpack
"P". That's why cperl fixed hash table security via proper means in
the collision resolution, not via a slow hash function, and not via
order hiding as perl5 believes in.
...
=item Scalar-List-Utils 1.46_07
