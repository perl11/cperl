...
C<OP_ARGCHECK> are not used, cperl still uses C<OP_SIGNATURE> only.
...
inflateUndermine: subvert arg conditionally used/unused
L<RT #120207|https://rt.cpan.org/Ticket/Display.html?id=120207>

two gcc6-found problems
L<RT #112829|https://rt.cpan.org/Ticket/Display.html?id=112829>

fix deflateParams for zlib > 1.2.8
Tests broken with zlib-1.2.10
L<RT #119762|https://rt.cpan.org/Ticket/Display.html?id=119762>
...
F<t/run/dtrace.t> is inheritently unsafe to run in parallel with
C<fresh_perl>, as it causes segfaults on darwin. We added locking, but
also re-arranged the order of parallel testing, running this test on
darwin before all the comp tests (we have ~10s for this), before all
the conflicting run tests.
