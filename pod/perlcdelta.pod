...
=head2 Detect, protect and warn against hash flood DoS
...
performs a sleep(2) to limit the impact.
...
Calls C<warn("Movable-Type CVE-2015-1592 Storable metasploit attack")>,
...
variants.  This is just a dumb match against existing exploits in the
wild, but not future variants.
Calls C<warn("metasploit reverse shell payload")>, but do not protect
against it. This warning is thrown even without C<-w>.
...
The new warning "Hash flood" was added.
See L</"Protect and warn on hash flood DoS">.

=item *

The new warning "metasploit reverse shell payload" was added,
detecting the existing metasploit/libxploit reverse shells.
See L</"Warn on metasploit reverse shell">
