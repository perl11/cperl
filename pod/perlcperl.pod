...
YAML). These modules usually get a +1 major version bump, so you
cannot easily override them accidently with worse cpan updates.  Such
as EUMM, bignum or Test2.
...
optimizable perl6-like object system in core.  Provide a simple MOP
for reflection purposes, bot not for overrides, i.e. metaobjects for
classes, proper class and method syntax, anonymous classes by pointer
not name, proper multi dispatch with types, roles, class and method
lookup by pointer, not by name.  Create native optimized shapes via
mixins as in p2 (an enhanced bless). Lexical methods are private,
optimize dispatch for single inheritance. i.e the convenient class
syntax extends on a single class only, classes are finalizable by the
calling application.
