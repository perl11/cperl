...
function instead.  Note: availability of I<undump> or I<UNEXEC> is
platform specific and may not be available for a specific port of
Perl.

In B<cperl>, C<CORE::dump> and C<-u> are very convenient ways to
create simple, unoptimized binaries, much faster than with B<perlcc>.
