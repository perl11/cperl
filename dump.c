...
#define FREQ_MAX ((int)(C_ARRAY_LENGTH(freq) - 1))
	...
	if (CvOUTSIDE(sv)
        ...
        if (CvHASSIG(sv)) {
            Perl_dump_indent(aTHX_ level, file, "  SIGOP = 0x%" UVxf, PTR2UV(CvSIGOP(sv)));
            if (CvSIGOP(sv))
                PerlIO_printf(file, "\t(%" SVf ")\n", SVfARG(signature_stringify((OP*)CvSIGOP(sv), (CV*)sv)));
        }
	break;
...
Perl_padlist_dump(pTHX_ PADLIST * padl)
{
    ...
    if (max == 1 || (!max && PadlistARRAY(padl))) {
