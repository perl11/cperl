...
#define PL_lex_sub_op           (PL_parser->lex_sub_op)
...
#   define UTF cBOOL((PL_linestr && DO_UTF8(PL_linestr)) \
                     || ( !(PL_parser->lex_flags & LEX_IGNORE_UTF8_HINTS) \
                          && (PL_hints & HINT_UTF8)))
#endif
...
/*
=for apidoc notify_parser_that_changed_to_utf8
            ...
            if (cfgv) {
                ...
                if (tmplen2 + 2 <= sizeof smallbuf)
                    ...
                    if (UNLIKELY(len > I32_MAX))
                        Perl_croak(aTHX_ "panic: name too long (%" UVuf ")",
                                   (UV) len);
                }
...
S_scan_const(pTHX_ char *start)
{
    ...
    assert(PL_lex_inwhat != OP_TRANSR);
    if (PL_lex_inwhat == OP_TRANS && PL_lex_sub_op) {
        /* If we are doing a trans and we know we want UTF8 set expectation */
        d_is_utf8   = OpPRIVATE(PL_lex_sub_op)
                     & (OPpTRANS_FROM_UTF|OPpTRANS_TO_UTF);
        s_is_utf8  = OpPRIVATE(PL_lex_sub_op)
...
S_scan_inputsymbol(pTHX_ char *start)
{
    ...
    const char * const e = PL_tokenbuf + sizeof PL_tokenbuf; /* end of it */
    ...
    */
    ...
    /* allow <Pkg::VALUE> and until 5.27.3 <Pkg'VALUE> */
    ...
    )
{
	...
	*to = '\0';
    ...
    if (d_is_utf8 || (IN_ENCODING && !re_reparse))
        SvUTF8_on(sv);
    if (d_is_utf8)
	SvUTF8_on(sv);
