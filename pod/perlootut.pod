...
L<Moose|http://search.cpan.org/dist/Moose/> bills itself as a
"postmodern object system for Perl 5". Don't be scared, the
"postmodern" label is a callback to Larry's description of Perl as
...
L<Mouse|http://search.cpan.org/dist/Mouse/> is the fast, smaller
variant of C<Moose> and the currently recommended OO system for perl5.

C<Moose> has a large compile-time penalty. Though significant progress
has been made over the years, the compile time penalty is a
non-starter for some very specific applications. If you are writing a
command-line application or CGI script where startup time is
essential, you may not be able to use C<Moose> (we recommend that you
instead use persistent Perl executing environments like "FastCGI" for
the latter, if possible).

C<Mouse> is a C<Moose> compatible object system, which aims to
alleviate this penalty by providing a subset of C<Moose>'s
functionality.

C<Mouse> is also going as light on dependencies as possible. C<Mouse>
currently has no dependencies except for building/testing
modules. C<Mouse> also works without XS, although it has an XS backend
to make it much faster.

Compatibility with C<Moose> has been the utmost concern. The sugary interface
is highly compatible with C<Moose>. Even the error messages are taken from
C<Moose>. The C<Mouse> code just runs its test suite 4x faster.

=head3 Moo

If you try C<Moose> and find that one of these issues is preventing
you from using C<Moose>, we encourage you to consider
L<Moo|http://search.cpan.org/dist/Moo/> next. C<Moo> implements a
subset of C<Moose>'s functionality in a simpler package.  For most
features that it does implement, the end-user API is I<identical> to
C<Moose>, meaning you can switch from C<Moo> to C<Moose> quite easily.
...
L<Class::Accessor|http://search.cpan.org/dist/Class::Accessor/> is the
polar opposite of C<Moose>. It provides very few features, nor is it
self-hosting.
...
Then, we have
L<Class::Tiny|http://search.cpan.org/dist/Class::Tiny/>. This module
truly lives up to its name. It has an incredibly minimal API and
absolutely no dependencies on any recent Perl. Still, we think it's a
lot easier to use than writing your own OO code from scratch.
...
roles with L<Role::Tiny|http://search.cpan.org/dist/Role::Tiny/>.
...
L<Dios|http://search.cpan.org/dist/Dios/> is the newest OO for
perl5. It provides the currently best feature set for a modern object
system, with a decent performance and is implemented via a nice but
slow macro system.  It has the closest remembrance to the perl6 and
syntax and type system.  But it relies in C<Object::InsideOut>.

=head2 base/fields

This was the default, old object system until 5.10. Before it was
compile-time optimized with pseudo-hashes allowing types and
compile-time optimizations of field accessors.
    ...
    my MyClass $foo = MyClass->new;
    $foo->{zap} = 24;

    # This will access the field foo per index 0, not per hash lookup.
    $foo->{foo} = 0;

use L<base> establishes a compile-time inheritance, the base classes.
use L<fields> establishes compile-time fields per class, which came
out of fashion since the removal of optimized pseudo-hashes with 5.10.
Before it guaranteed field-access of typed objects per index, not per hash,
and convenient typed classes and restricted hashes to detect wrong field
and method names at compile-time.

Compile-time composition of roles was also possible, and the DOES and
NEXT methods were supported.
...
ecosystem, and a thriving user base. C<Mouse> is the recommended fast
variant of C<Moose>. We also covered L<Moo> briefly.  C<Moo> is
C<Moose> lite, and a reasonable alternative when Moose doesn't work
for your application.

=item * L<Mouse>

The fast drop-in replacement for Moose, if Moose is too slow for you.
...
Has a familiar syntax, needs faster macros, uses internally insideout
objects, which are much faster to use, but come with security
problems, and is very new.

=item * L<base> / L<fields>

The old simple OO, severly restricted since the removal of pseudo-hashes.
...
For bigger projects, L<Moose> and L<Mouse> provide a rich set of
features that will let you focus on implementing your business
logic. L<Moo> provides a nice alternative to L<Moose> when you want a
lot of features but need faster compile time or to avoid XS.

We encourage you to play with and evaluate L<Moose>, L<Mouse>, L<Moo>,
L<Class::Accessor>, L<Class::Tiny> and L<Dios> to see which OO system
is right for you.
