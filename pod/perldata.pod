...
B<cperl> implements the L<Moderately Restrictive level
|http://www.unicode.org/reports/tr39/#General_Security_Profile> for
unicode identifiers.

With cperl all utf8 identifiers and literals are parsed and stored as
normalized NFC variants, which prevents from various TR39 and TR36
unicode confusable and spoofing security problems.  However, prio to
cperl 5.27.1 dynamically created symbols via string refs were not
normalized.  C<${"$decomposed"}> stayed decomposed. Since cperl 5.27.1
under use strict names the dynamic run-time rules match the static
parser rules.  Normalization is similar to Perl 6 and Python 3, but
Perl 6 normalized to their own NFG format and Python 3 normalizes to
the non-canonical NFKC format.  See
L<http://www.unicode.org/reports/tr36/> and
L<http://www.unicode.org/reports/tr39>.

Also with cperl mixed scripts in unicode identifiers are forbidden,
but they can be declared via C<use utf8 (Scriptnames...);>.  The
'Common', 'Latin' and 'Inherited' scripts are always allowed and don't
need to be declared. Also the following combinations are allowed
without any declaration: Latin + Hangul + Han (:Korean), Latin +
Katakana + Hiregana (::Japanese) and Latin + Han + Bopomofo (:Hanb).
And some more recommended and aspirational scripts, which are not
excluded, except Cyrillic and Greek. See L<utf8/Valid scripts>.
...
to parsers errors.  See L<utf8/"Valid scripts">,
L<http://www.unicode.org/reports/tr36/#Mixed_Script_Spoofing> and
...
=> Error: Invalid script Balinese in identifier ᭅ for U+1B45

    use utf8 'Balinese';
    my $ᭅ = 1; # \x{1b45} BALINESE LETTER KAF SASAK
    print "ok";

Please avoid the declaration of Cyrillic and Greek scripts in one
source file.
...
There are two package separators in Perl, but only one in cperl: A
double colon (C<::>) and a single quote (C<'>).  Normal identifiers
can start or end with a double colon, and can contain several parts
delimited by double colons.  Single quotes have similar rules, but
with the exception that they are not legal at the end of an
identifier: That is, C<$'foo> and C<$foo'bar> are legal, but
C<$foo'bar'> is not.
In cperl only the double colon (C<::>) is legal, the perl4 package
seperator C<'> was illegal, and since 5.26c is part of an identifer,
not a seperator - as in perl6.
Full support for the single quote C<'> as part of an identifer came
with 5.28c, and matches now the behavior of perl6.
...
Differences, when being expanded in strings:
Note that hashes are not expanded, e.g. C<%file's> stays C<%file's>,
only scalars and arrays are.
...
If the identifier is preceded by a sigil -- that is, if the identifier
is part of a variable name -- it may optionally be enclosed in braces.
...
Prior to cperl v5.25.2c mixed-script unicode identifiers were allowed,
and identifiers were not normalized to NFC.

Since 5.16 up and until cperl v5.25.2c, binary names were silently
allowed in names, such symbols and packages. Before 5.16 names were
silently stripped, since v5.25.2c names are stripped with a prominent
security warning.

Since cperl 5.27.1 under use strict names, the parsing rules are
enforced even with dynamically created identifiers.
...
L<coretypes|/"Constant and typed lexical variables"> there's no place
to declare a scalar variable to be of type "string", type "number",
type "reference", or anything else.  Because of the automatic
conversion of scalars, operations that return scalars don't need to
care (and in fact, cannot care) whether their caller is looking for a
string, a number, or a reference.  Perl is a contextually polymorphic
language whose scalars can be strings, numbers, or references (which
includes objects).  Although strings and numbers are considered pretty
much the same thing for nearly all purposes, references are
...
But note that this only works for hashes with already existing keys, and
sizes lower than the existing number of keys are silently ignored.
...
X<:const> X<type> X<coretypes>

You can declare lexical my, our or state variables with the C<:const>
attribute as readonly. C<:const> is currently only available with the
L<perlcperl> variant.
...
module, just not backwards compatible and C<:const> can throw errors
...
With the cperl variant C<coretypes> is implicitly loaded
and you can declare the type of a lexical variable as C<int>,
C<num> or C<str>. C<num> uses the internal floating point
represention, which might be better or worse than IEEE doubles.

Such native typed variables can not be magic, i.e. not tied, and type
violations such as stringification of int or num, setting to undef
...
the type of all values.  The :const attribute for arrays specifies
...
Hash keys are always of type str, so the hash type declaration specifies
the type of all values.  The const attribute for hashes specifies
...
A slice always behaves like the list of its elements. I.e. a hash slice
as for loop list modifies the elements, but a hash slice as subroutine
arguments not.
...
Lexical array declarations can contain the number of elements, which
will lead to compile-bound checks, and faster run-time ops, omitting
the otherwise necessary out-of-bounds check.
...
cperl-only, see L<perlcperl/Typed and shaped arrays>.

The C<:const> attribute also creates a shaped array.

    my @a :const = (1,2);  # shaped as int @a[2]

See L<perlcperl/:const for all>.
