...
three-dot variant of binary flip-flop C<...> operator. (See
L<perlop/"Range Operators">).  One is a statement and the other an
...
With cperl the C<...> in a signature will denote varargs, and the
C<...> in a body will then not be interpreted as ellipsis
C<Unimplemented> error, but as placeholder for the unnamed rest arguments.
Usage of C<...> as ellipsis is therefore discouraged.
