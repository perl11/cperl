...
Debugging with private archlibs and exename:
    ...
    ./Configure -sder -Dusedevel -Dusecperl -DDEBUGGING \
      ...
      -Darchname="$archname" -Darchlib="/usr/local/lib/cperl/5.23.0/$archname" \
      -Dsitearch="/usr/local/lib/cperl/site_cperl/5.23.0/$archname" \
      -Dperlpath="/usr/local/bin/cperl5.23.0$exesuff" -Dstartperl="#!/usr/local/bin/cperl5.23.0$exesuff" \
    make -s -j4 ECHO=true
    make -s -j4 ECHO=true test
    sudo make install
...
=head2 Typed and shaped hashes (not yet)
...
Optimize HEK for faster hash comparison, and use it for native str
types, which might benefit from its immutability and the pre-calculated
hash, len and utf8 fields.

Check readonly support for PL_strtab for the compiler.
Builtin ro ph + dynamic as in warnings XS.

Better symbol table: Check converting the GV stash tree of hashes into
a single global data structure, not a nested hash of hashes: Hash, AVL
tree, Trie (TST or R² TST), Patricia trie or DAFSA (Deterministic
acyclic finite state automaton) for faster dynamic variable and
function name lookup. No binary names, all as UTF8. Maybe restrict to
ASCII to limit the trie memory (array of 26 vs 256).  Stashes point
then to trie nodes and need a HV check.  Optionally provide partial
read-only support for the compiler, as for PL_strtab.
...
types, signatures, classes + mop, hash "security theatre", cow system,
"unicode"(=binary) symbols, bindings as refalias, double readonly
system and many more.  Additionallly most bugs don't get fixed.
...
encourage criticism on bad design and implementation.  Maintainers
stay civil and don't call critics I<"jerks"> or I<"assholes">, unlike
in the p5p community.  We follow software development best practices,
we develop in branches only, and only if the branch passes the test
suite on all major platforms it can be merged.  Pushing directly to
master is forbidden, but because it is a fork we do have to rebase
frequently all our branches.  Use the provided rebase helper scripts
F<cp-*>. We do use build servers and performance regression tests.  We
...
mostly until 5.20 and 5.22, and 5.20 alone was not worth to update
cperl to. The previous stable release was 5.8.9 and the one before was
5.6.2.  So we do not know which will be the next stable cperl
release. It could be 5.24, but usually upstream development makes
grave mistakes which are never acknowledged and needs several
following releases to overcome. So something like 5.30 sounds
realistic.  So far we have detected 2 grave API mistakes in
5.22.0. But we do merge all official p5p releases into our development
branch monthly.
...
=item classes (not yet)

A true perl6-like object system is in work, but not yet enabled.  It
supports the perl6-like syntax for class, method, has, roles (mixins,
compile-time composable classes), method modifiers (:before, :after,
:around), multiple type dispatch (multi, no need to overload methods),
finalizable classes (compile-time optimizations) and easier perl6-like
syntax, which closely resembles C<Moose> without its massive overhead.
...
still strip the AST, but needs to keep all COW strings dynamic.

With cperl we can again compile to more efficient code, with >30%
memory savings.

See L<B::C|http://search.cpan.org/dist/B-C/>

=item company friendly
