...
or:
    ...
    for t in deb rpm osxpkg solaris pkgin; do
      fpm -s dir -t $t -C cperl -p ../cperl-5.25.2-inst/ \
        --version 5.25.2 \
        -m rurban@cpanel.net \
...
internal test files.
    ...
    mkdir ../cperl-5.25.2-build
    cp -rp .git ../cperl-5.25.2-build/
    cd ../cperl-5.25.2-build
    git reset --hard
    git clean -dxf
...
.rpm, osx .pkg, bsd and solaris packages, but not yet windows .msi
    ...
    fpm -s dir -t deb -n cperl -v 5.25.2-1 -m 'rurban@cpanel.net' \
    ...
    fpm -s dir -t rpm -n cperl -v 5.25.2 -m 'rurban@cpanel.net' \
        -d 'zlib' -d bzip2-libs -d 'gdbm' -d 'db4' \
        --url http://perl11.org/cperl/rpm/ \
        -C ../cperl-5.25.2-inst .

    # on an amd64 osx machine
    fpm -s dir -t osxpkg -n cperl -v 5.25.2 -m 'rurban@cpanel.net' \
        --url http://perl11.org/cperl/osxpkg/ \
        -C ../cperl-5.25.2-inst .

    # on i386 and amd64 solaris machines
    fpm -s dir -t solaris -n cperl -v 5.25.2 -m 'rurban@cpanel.net' \
        --url http://perl11.org/cperl/solaris/ \
        -C ../cperl-5.25.2-inst .

    # on i386 and amd64 bsd machines
    fpm -s dir -t pkgin -n cperl -v 5.25.2 -m 'rurban@cpanel.net' \
...
Add the debs to the pool:
    ...
    aptly repo add cperl ../../cperl_5.25.2-1_{i386,amd64}.deb
